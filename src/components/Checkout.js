  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        <Button variant="contained" onClick={()=>{addAddress(token,newAddress)}}>Add</Button>
        <Button variant="text" onClick={()=>{handleNewAddress({isAddingNewAddress:false})}}>Cancel</Button>
      </Stack>
    </Box>
  );
};
...
const Checkout = () => {
  ...
  const getAddresses = async (token) => {
    ...
    try {
     ...
     // console.log("this is response from get adresses", response.data);
     // console.log("this is value of state variable adresses,", addresses);
      return response.data;
    } catch {
      enqueueSnackbar(
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      let postdataaddress=await axios.post(config.endpoint+`/user/addresses`,{"address":newAddress.value},{headers:{'Authorization':`Bearer ${token}`,'Content-Type':'application/json'}})
      //console.log("this is response of adding address",postdataaddress.data);
      setNewAddress({isAddingNewAddress:false,value:""});
      setAddresses({...addresses,all:postdataaddress.data})
      return postdataaddress.data;
    } catch (e) {
      if (e.response) {
        enqueueSnackbar(e.response.data.message, { variant: "error" });
      } else {
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      let deleteaddr=await axios.delete(config.endpoint+`/user/addresses/${addressId}`,{headers:{'Authorization':`Bearer ${token}`}})
      //console.log(deleteaddr.data);
      setAddresses({all:deleteaddr.data});
      return deleteaddr.data;
    } catch (e) {
      if (e.response) {
        enqueueSnackbar(e.response.data.message, { variant: "error" });
      } else {
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    if(validateRequest(items,addresses)){
      let checkoutresponse= await axios.post(config.endpoint+`/cart/checkout`,{"addressId":addresses.selected},{headers:{'Authorization':`Bearer ${token}`}});
      console.log(checkoutresponse);
      enqueueSnackbar("Order placed successfully",{variant:"success"})
      history.push('/thanks');
      let wallet=localStorage.getItem("balance");
      wallet=wallet-getTotalCartValue(items);
            ...
            <Box>
              ...
              {/* <Typography my="1rem"> */}
                {addresses.all.length>0
                  ? addresses.all.map((items) => (
                      <div key={items._id}
                        className={
                          addresses.selected === items._id
                            ? "address-item selected"
                            : "address-item not-selected"
                        }
                        onClick={() => {
                           setAddresses({...addresses,selected: items._id });
                          //console.log("this is addresses on button click",addresses);
                        }}
                      >
                        {items.address}
                        <Button variant="outlined" endIcon={<Delete />} onClick={()=>{deleteAddress(token,items._id)}}>
                          Delete
                        </Button>
                      </div>
                    ))
